...
public class GlobalLoginOrRegisterFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        try {
            loginUserInfo = TokenParseUtil.parseFromToken(token);
        } catch (Exception ex) {
    ...
    private String getTokenFromAuthorityCenter(ServerHttpRequest request, String uriFormat) {
        ...
        Future<ServiceInstance> future =
                executorService.submit(() -> loadBalancerClient.choose(CommonConstant.AUTHORITY_SERVICE_ID));
        ...
        ResponseEntity<JwtToken> responseEntity = restTemplate.exchange(
                requestUrl,
                HttpMethod.POST,
